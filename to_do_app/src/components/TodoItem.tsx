...
const TodoItem: React.FC<TodoItemProps> = ({ todo, onToggle, onRemove }) => {
  return (
    <div className="flex w-1/2 border rounded justify-between items-center">
      <div onClick={() => onToggle(todo.id)} className="flex items-center justify-between p-2 cursor-pointer w-full">
        <p
          onClick={() => onToggle(todo.id)}
          className={
            todo.completed ? "line-through text-gray-500 cursor-pointer" : "cursor-pointer" 
          }
        >
          {todo.task}
        </p>
